...
if ANTHROPIC_API_KEY:
    exec(mid, f'{ENV} && openclaw config set env.vars.ANTHROPIC_API_KEY "{ANTHROPIC_API_KEY}"')
else:
    print("WARNING: ANTHROPIC_API_KEY not set, skipping LLM key configuration")
exec(mid, f"{ENV} && openclaw config set gateway.http.endpoints.chatCompletions.enabled true")
